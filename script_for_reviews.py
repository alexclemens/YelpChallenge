...
keys = ['food', 'inside', 'outside', 'drink', 'label', 'caption', 'food_and_caption', 'food_and_no_caption']
...
for curr_string in curr:
    ...
    if curr_string.get('label') == 'food':
    	...
    	dictionary['label'] += 1
    elif curr_string.get('label') == 'drink':
        dictionary['drink'] += 1
        dictionary['label'] += 1
...
print "Label is drink: " + str(dictionary['drink'])
